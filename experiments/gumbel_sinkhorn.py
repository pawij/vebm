...
if __name__ == "__main__":
    ...
    def unpack_params(params):
        assert params.shape == (2 * (n_bms)**2, )
        log_mu = np.reshape((params[:(n_bms)**2]), (n_bms, n_bms))
        logit_sigma = np.reshape((params[(n_bms)**2:]), (n_bms, n_bms))
        #FIXME: constrain sigma?
        #        sigma = sigma_min + (sigma_max - sigma_min) * logistic(logit_sigma)
        #FIXME: need to use logistic transform?
        log_sigma = np.log(logistic(logit_sigma))
    ...
    def q_entropy(log_sigmasq_P, temperature):
        #FIXME: check
        return gaussian_entropy(0.5 * log_sigmasq_P) + log_sigmasq_P.size * np.log(temperature)
    ...
    def variational_objective(params, t, return_dr=False):
        ...
        def gumbel_distance(log_mu_P, temperature_prior, temperature):
            ...
            arr = np.sum(np.log(temperature_prior) - 0.5772156649 * temperature_prior / temperature -
                         ...
                         np.exp(gammaln(1 + temperature_prior / temperature) - log_mu_P * temperature_prior / temperature)
    ...
    log_mu_P_post, log_sigmasq_post = unpack_params(variational_params)
